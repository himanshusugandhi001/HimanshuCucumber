package com.basic.com.basic.postUpdatedJsonSD;
...
//	@Given("^user hit the post request with \"([^\"]*)\" name$")
	@Given("^user hit the post request with \"([^\"]*)\" name and \"([^\"]*)\" value$")
	public void user_hit_the_post_request_with_json_data(String username , String job) throws IOException{
		
		//Read data from file
		String data = ""; 
	    data = new String(Files.readAllBytes(Paths.get("src/test/java/com/basic/com/basic/postFromTextFileSD/user.json"))); 
	    ...
	    String data1 = ""; 
	    data = new String(Files.readAllBytes(Paths.get("src/test/java/com/basic/com/basic/postFromTextFileSD/user.json"))); 
	    System.out.println("json in String ==== " +data1);
	    
	    //Update the data
	    JSONObject object1=new JSONObject(data);
	    object1.put("Ujob", job);
	    String newJsonStr1 = object.toString();
	    System.out.println("json in String ==== " +newJsonStr1);
	    
	    //Do POST request
	    resp = RestAssured.given().accept(ContentType.JSON).relaxedHTTPSValidation().
				when().body(newJsonStr1).post("https://reqres.in/api/users");
